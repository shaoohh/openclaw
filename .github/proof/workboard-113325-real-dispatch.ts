...
const PR_HEAD_SHA = "9f8c1a7853f01c98983c4dbecaa35e0276f5f723";
...
try {
  ...
  const withinGraceAt = expiresAt + CLAIM_RECLAIM_GRACE_MS - 30_000;
  console.log(
    `[trace] waiting_for_boundary wall_clock_ms=${Math.max(0, withinGraceAt - Date.now())}`,
  );
  await waitUntil(withinGraceAt);
  ...
  assertProof(
    Date.now() < expiresAt + CLAIM_RECLAIM_GRACE_MS,
    "the first dispatch must complete inside the late-heartbeat grace window",
  );
  ...
  console.log(
    "[trace] within_grace boundary_offset_ms=-30000 expired=true claim_present=true stale_status=running stale_execution=running started=0 worker_calls=0 capacity_reserved=true",
  );
